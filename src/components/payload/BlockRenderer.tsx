...
import { resolveMediaURL, tryGetPayloadClient, tryGetProductsCatalog, tryGetGlobals, toPayloadLocale, type ProductsCatalog } from "@/lib/payload";
...
const BlockRenderer = async ({ locale: stringLocale, blocks, productsCatalog: initialProductsCatalog }: BlockRendererProps) => {
  ...
  for (const block of blocks) {
    ...
    if (blockType === "features") {
      const slugs = ((block as any)?.featuredProducts?.slugs ?? []).map((i: any) => typeof i === "object" ? i?.slug : i).filter(Boolean);
  ...
  if (hasFeaturedProducts) {
    ...
    if (payload) {
      ...
      featuredViewDetailsCta = String(globals?.siteSettings?.ctaDefaults?.viewDetailsCta ?? "View Details");
  ...
  const renderBlock = async (block: any, index: number) => {
    try {
      ...
      if (blockType === "productsCatalog") {
          ...
          const globals = await tryGetGlobals({ payload, locale: payloadLocale, depth: 1 });
          const ctaFallback = globals?.siteSettings?.ctaDefaults?.viewDetailsCta ?? "View Details";
          const viewDetailsCta = String((block as any)?.viewDetailsCta ?? ctaFallback);
          ...
          const sortedSeries = (catalog.series ?? []).map((series) => {
            ...
            if (rankByID.size > 0) {
              items.sort((a: any, b: any) => {
                const ar = rankByID.get(String(a?.id ?? "")) ?? Number.POSITIVE_INFINITY;
                const br = rankByID.get(String(b?.id ?? "")) ?? Number.POSITIVE_INFINITY;
          ...
          const seriesOrderKeys = seriesOrderEntries
            ...
            .map((s: any) => typeof s === "object" ? (s?.id ?? s?.key ?? "") : s)
            .map((v: any) => String(v ?? ""))
            .filter(Boolean);
          ...
          const orderedSet = new Set(orderedSeries.filter(Boolean).map((s) => String(s.key)));
          const remaining = sortedSeries.filter((s: any) => !orderedSet.has(String(s?.key ?? "")));
      ...
      if (blockType === "caseStudiesList") {
          ...
          const normalizedCases = (res.docs as any[]).map((doc) => ({
            id: doc?.id,
      ...
      if (blockType === "features") {
          ...
          const featuredProductsSlugs = ((block as any)?.featuredProducts?.slugs ?? [])
      ...
      if (blockType === "about") {
        ...
        const sectionOneProps = {
          ...
          highlights: ((block as any)?.highlights ?? []).map((h: any) => h?.text).filter(Boolean),
          image: {
            src: resolveMediaURL((block as any)?.image) || "/images/about/company-mascot.png",
            alt: imageAlt,
          }
        };
      ...
      if (blockType === "richText") {
        return (
          <section key={`block-${blockType}-${index}`} className="py-8 md:py-12 lg:py-16">
            <div className="container">
              <div className="mx-auto w-full max-w-[1320px] rounded-2xl bg-white p-6 shadow-sm dark:bg-dark-2 md:p-10">
                {(block as any)?.blockName && (
                  <h2 className="mb-8 border-b border-stroke pb-6 text-2xl font-bold text-black dark:border-stroke-dark dark:text-white sm:text-3xl">
                    {(block as any).blockName}
                  </h2>
                )}
                <RichText data={(block as any)?.content} className="prose max-w-none dark:prose-invert prose-headings:text-black dark:prose-headings:text-white prose-p:text-body-color dark:prose-p:text-body-color-dark prose-li:text-body-color dark:prose-li:text-body-color-dark marker:text-primary" />
              </div>
            </div>
          </section>
        );
      ...
      if (blockType === "imageGallery") {
        const images = Array.isArray((block as any)?.images) ? (block as any).images : [];
        const title = (block as any)?.title;
        return (
          <section key={`block-${blockType}-${index}`} className="py-12 md:py-16 lg:py-20">
            <div className="container">
              {title ? (
                <h2 className="mb-8 text-2xl font-semibold text-black dark:text-white">{title}</h2>
              ) : null}
                ...
                {images.map((item: any, imageIndex: number) => {
                  ...
                  return (
                    <figure
                      ...
                      className="overflow-hidden rounded-2xl border border-black/5 bg-white shadow-sm dark:border-white/10 dark:bg-gray-dark/80"
                    >
                      ...
                      {caption ? (
                        <figcaption className="px-4 py-3 text-sm text-body-color dark:text-body-color-dark">
                          {caption}
                        </figcaption>
                      ) : null}
                    </figure>
      ...
      if (blockType === "contact") {
        ...
        const intentionOptions = Array.isArray(cmsForm?.intentionOptions)
          ? cmsForm.intentionOptions.map((item: any) => item?.option).filter(Boolean)
          : [];
        return (
          <Contact
            key={`block-${blockType}-${index}`}
      ...
      console.error(`[BlockRenderer] Failed to render block at index=${index}, type=${block?.blockType ?? "unknown"}`, error);
      return null;
    }
  };
