...
状态：`ARCHIVED`（最近审查：2026-04-15）

> 说明：
> - 本文档包含大量“特定实例/特定账号/特定网络环境”的操作记录（例如实例名、IAP 参数、Windows 路径等），目前仅保留备查。
> - 当前仓库的推荐生产发布方式以阿里云 ECS 一键脚本为主；通用生产结构请参考 `docs/archive/deploy/docker-production.md`。

本文档面向 GCP 实例部署场景，全程采用 **本地 WSL 一键构建部署包 -> 分片传输 -> 云端拼合部署** 的高容错链路。
...
在 WSL 终端中（需处于项目根目录下），直接执行一键打包命令（init 包含 db/media 快照；update 为轻量更新包）：
...
# 将这些碎片提取到 Windows 宿主机的 Downloads 下载文件夹中
cp bundle_part_* /mnt/c/Users/Admin/Downloads/
```

*(此时 Windows 的 Downloads 文件夹下会出现 bundle_part_aa、bundle_part_ab 等多个小文件。)*
...
在 PowerShell 中打开一个新标签，使用 `gcloud compute ssh` 连入服务器。具体命令格式如下：
...
* **命令参数详解**：
  ...
  * `--zone=us-central1-c`：指定要连接的这台 GCP 服务器所在的地理可用区。
  * `--tunnel-through-iap`：核心安全参数。它强制流量通过谷歌的 Identity-Aware Proxy (IAP) 内网专线隧道进行转发，即使实例没有公网 IP 也可安全连入并避免防火墙阻断。
...
完成解压后，当前所处目录为 `proj-unihome-deploy-bundle/`，推荐使用一键脚本完成首次部署或后续更新。

### 1. 配置最终公网域名
生产环境推荐把环境变量固定在 `/opt/proj_unihome/shared/.env.production`，避免后续更新覆盖密钥与数据库口令。

将 `NEXT_PUBLIC_SERVER_URL` 改为计划对外绑定的真实前端正式域名（例如 `https://xxx.xxx`，注意末尾不要加 `/` 斜杠）。暂时没有的话可先留回环 IP：
```bash
mkdir -p ../shared
test -f ../shared/.env.production || cp .env.production ../shared/.env.production
...
3. 只要无明显错误，容器便会固定监听本机 `127.0.0.1:3005`。在此基础上，可利用外层的 Nginx / Caddy 设施，把域名的 `80/443` 流量转发到 `3005` 进行最终交付。
